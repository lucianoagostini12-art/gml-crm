                    ...
                    <DialogHeader className="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex flex-row items-center justify-between shrink-0">
                        <div className="flex items-center gap-6">
                            ...
                            <div
                                ...
                                className={`h-16 w-16 rounded-2xl flex items-center justify-center shadow-md cursor-pointer hover:scale-105 transition-transform active:scale-95 ${isPass ? 'bg-purple-100 text-purple-600' : 'bg-green-100 text-green-600'
                                        ...
                                        <ScrollArea className="flex-1 p-8 text-slate-900 bg-slate-50/50 min-h-0">
                                            ...
                                            {realChat.length === 0 ? <p className="text-center text-xs text-slate-400 mt-10">Inicio del chat.</p> :
                                                realChat.map((msg: any, i: number) => (
                                                    <ChatBubble key={i} text={msg.text} user={msg.sender} time={new Date(msg.created_at).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Argentina/Buenos_Aires' })} isMe={msg.sender === (currentUser || "Administración")} />
                                                ))
                                            }
                                        </ScrollArea>
