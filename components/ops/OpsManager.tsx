                            ...
                            <Popover open={isFilterOpen} onOpenChange={setIsFilterOpen}>
                                ...
                                <PopoverContent align="end" className="w-[340px] p-4 shadow-xl border-slate-200">
                                    <div className="space-y-4">
                                        ...
                                        <div className="space-y-1"><Label className="text-xs font-bold text-slate-500 uppercase">Sub-Estado</Label><Select value={filterSubState} onValueChange={setFilterSubState}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="all">Todos</SelectItem>{uniqueSubStates.map(sub => <SelectItem key={sub} value={sub || "sin_dato"}>{sub}</SelectItem>)}</SelectContent></Select></div>
                                        <div className="space-y-1"><Label className="text-xs font-bold text-slate-500 uppercase">Vendedor</Label><Select value={filterSeller} onValueChange={setFilterSeller}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="all">Todos</SelectItem>{uniqueSellers.map(s => <SelectItem key={s} value={s || "sin_dato"}>{s}</SelectItem>)}</SelectContent></Select></div>
