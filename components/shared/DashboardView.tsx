...
export function DashboardView() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const { data: leads } = await supabase.from('leads').select('*').eq('agent_name', CURRENT_USER)
            if (leads) {
                const total = leads.length
                const contactados = leads.filter(l => l.status === 'contactado' || l.calls > 0).length
                const cotizados = leads.filter(l => l.status === 'cotizacion' || l.quoted_price).length
                const vendidos = leads.filter(l => l.status === 'vendido').reduce((sum, l) => sum + (l.capitas || 1), 0)
                const todayStr = new Date().toLocaleDateString()
                const callsToday = leads.filter(l => new Date(l.last_update).toLocaleDateString() === todayStr && l.calls > 0).length
                ...
                const sourceArray = Object.entries(sourcesMap).map(([name, data]) => ({
                    name, total: data.total, won: data.won,
    ...
    // LÓGICA DE NIVELES (3, 6, 9... hasta 25 Leyenda)
        ...
        <div className="p-6 h-full overflow-y-auto max-w-6xl mx-auto text-slate-900 dark:text-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                <Card className="dark:bg-slate-900 dark:border-slate-800 shadow-sm">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">Gestión Hoy</CardTitle><Activity className="h-4 w-4 text-slate-400" /></CardHeader>
                    <CardContent><div className="text-3xl font-bold">{stats.callsToday}</div><p className="text-xs text-slate-500">Clientes tocados</p></CardContent>
                </Card>
                <Card className="dark:bg-slate-900 dark:border-slate-800 shadow-sm">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">Cotizaciones</CardTitle><DollarSign className="h-4 w-4 text-yellow-500" /></CardHeader>
                    <CardContent><div className="text-3xl font-bold">{stats.cotizados}</div><p className="text-xs text-slate-500">Enviadas este mes</p></CardContent>
                </Card>
                <Card className="dark:bg-slate-900 dark:border-slate-800 shadow-sm">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">Ventas (Cápitas)</CardTitle><TrendingUp className="h-4 w-4 text-green-500" /></CardHeader>
                    <CardContent><div className="text-3xl font-bold">{stats.vendidos}</div><p className="text-xs text-slate-500">Vidas aseguradas</p></CardContent>
                </Card>
                <Card className="dark:bg-slate-900 dark:border-slate-800 shadow-sm">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">Efectividad</CardTitle><BarChart className="h-4 w-4 text-blue-500" /></CardHeader>
                    <CardContent><div className="text-3xl font-bold">{stats.contactados > 0 ? ((stats.vendidos / stats.contactados) * 100).toFixed(1) : 0}%</div><p className="text-xs text-slate-500">Cierre sobre Contactados</p></CardContent>
                </Card>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    <h3 className="text-lg font-bold mb-4 flex items-center gap-2 text-slate-700 dark:text-slate-200"><PieChart className="h-5 w-5 text-purple-500"/> Canales</h3>
                    <div className="space-y-3">
                        {sourceStats.map((source) => (
                            <div key={source.name} className="flex items-center justify-between p-3 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800">
                                <div><p className="font-bold text-sm">{source.name}</p><p className="text-xs text-slate-500">{source.total} datos</p></div>
                                <div className="text-right"><span className="text-green-600 font-bold text-sm">{source.rate}%</span><div className="h-1 w-16 bg-slate-200 rounded-full mt-1 overflow-hidden"><div className="h-full bg-green-500" style={{width: `${source.rate}%`}}></div></div></div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div>
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-bold flex items-center gap-2 text-slate-700 dark:text-slate-200"><History className="h-5 w-5 text-blue-500"/> Últimos Toques</h3>
                        {auditLogs.length > 1 && (
                            <Button variant="ghost" size="sm" className="text-slate-500 hover:text-slate-800 dark:hover:text-white" onClick={() => setIsAuditExpanded(!isAuditExpanded)}>
                                {isAuditExpanded ? <span className="flex items-center text-xs">Menos <ChevronUp className="h-4 w-4 ml-1"/></span> : <span className="flex items-center text-xs">Ver historial <ChevronDown className="h-4 w-4 ml-1"/></span>}
                            </Button>
                        )}
                    </div>
