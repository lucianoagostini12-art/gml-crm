    ...
    `}>
            ...
            <CardHeader className="p-2 pb-0 flex flex-col items-start space-y-0 relative">
                ...
                <div className="w-full">
                    ...
                    {/* ✅ NUEVO: Etiquetas IA de Sofía */}
                    {lead.ai_labels && lead.ai_labels.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1.5">
                            {lead.chat_source === 'sofia_ai' && (
                                <Badge className="bg-violet-100 text-violet-700 border-violet-200 text-[8px] px-1 py-0 font-bold">
                                    🤖 Sofía
                                </Badge>
                            )}
                            {lead.ai_labels.slice(0, 2).map((label, idx) => (
                                <Badge
                                    key={idx}
                                    variant="outline"
                            ...
                            {lead.ai_labels.length > 2 && (
                                <span className="text-[8px] text-slate-400 font-medium">+{lead.ai_labels.length - 2}</span>
                            )}
                        </div>
                    )}
                </div>
