...
type Msg = { user: string; text: string; isMe: boolean }

const INITIAL_MSG: Msg = {
  user: "Sofía",
  text: "¡Hola! Soy Sofía de GML. ¿En qué te puedo ayudar? 😊",
  isMe: true,
}
...
export default function TestBotPage() {
  ...
  // 🔒 Lock inmediato: evita doble envío antes de que React actualice loading
  ...
  const handleSend = async () => {
    ...
    // 🔒 Si ya hay una request en vuelo, no mandamos otra
    ...
    const userMsg: Msg = { user: "Yo", text, isMe: false }
    ...
    try {
      ...
      // 3) Agregamos respuesta en el chat (SIN alerts)
      if (response?.success) {
        setMessages((prev) => [...prev, { user: "Sofía", text: response.text, isMe: true }])
      } else {
        // Si falla, que Sofía lo diga adentro del chat (más premium)
        setMessages((prev) => [
          ...prev,
          {
            user: "Sofía",
            text: response?.text || "Uy, se me trabó un segundo 🙏 ¿Me repetís?",
            isMe: true,
          },
        ])
      }
      ...
      setMessages((prev) => [
        ...prev,
        { user: "Sofía", text: "Uy, se me trabó un segundo 🙏 ¿Me repetís?", isMe: true },
      ])
    } finally {
      setLoading(false)
      ...
      <div className="p-3 bg-[#f0f2f5] border-t">
        <div className="flex gap-2 items-end bg-white p-2 rounded-2xl border border-slate-200">
          <Input
            ...
            onKeyDown={(e) => {
              // Evita doble envío raro y respeta IME (teclados)
          ...
          <Button
            ...
            disabled={loading}
            size="icon"
            className="bg-purple-600 rounded-xl h-10 w-10 shrink-0 mb-0.5"
          >
            <Send size={18} />
          </Button>
        </div>

        <p className="text-[10px] text-center text-slate-400 mt-2">Ambiente de Pruebas • GML Sales Group</p>
      </div>
    </div>
  )
}
